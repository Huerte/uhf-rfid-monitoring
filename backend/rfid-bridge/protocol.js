...
export const EnumG = {
  ...
  InventoryMode_Single: 0x00,
  InventoryMode_Inventory: 0x01,

  WriteArea_Reserved: 0x00,
  WriteArea_Epc: 0x01,
  WriteArea_Tid: 0x02,
  WriteArea_UserData: 0x03,

  ParamFilterArea_EPC: 0x00,
  ParamFilterArea_TID: 0x01,
  ParamFilterArea_UserData: 0x02,

  ParamTidMode_Auto: 0x00,
  ParamTidMode_Fixed: 0x01,
};

export function buildAntennaMask(antennaList) {
  let mask = 0;
  for (const ant of antennaList) {
    const bit = EnumG[`AntennaNo_${ant}`];
    if (bit !== undefined) {
      mask |= bit;
    }
  }
...
export function buildFrame(pType, pVersion, baseType, msgId, cData = Buffer.alloc(0)) {
  ...
  lenBuf.writeUInt16BE(dataLen, 0);

  const payloadToCrc = Buffer.concat([headerPart, lenBuf, cData]);
  const crcBuf = crc16(payloadToCrc);
...
/**
 * Parse an incoming byte frame from reader
 */
export function parseFrame(buffer) {
  if (buffer.length < 9) return null; // Min frame size
  if (buffer[0] !== HEADER_BYTE) return null;

  const pType = buffer[1];
  const pVersion = buffer[2];
  const baseType = buffer[3];
  const msgId = buffer[4];
  const dataLen = buffer.readUInt16BE(5);

  const totalLength = 1 + 4 + 2 + dataLen + 2;
  if (buffer.length < totalLength) return null; // incomplete frame

  const payload = buffer.subarray(7, 7 + dataLen);
  const expectedCrc = buffer.subarray(7 + dataLen, totalLength);

  const checksumPart = buffer.subarray(1, 7 + dataLen);
  const calculatedCrc = crc16(checksumPart);

  const isValidCrc = expectedCrc.equals(calculatedCrc);
